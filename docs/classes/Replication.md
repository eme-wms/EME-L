...
| Метод | Аргументы | Возвращает | Описание |
|-------|-----------|------------|----------|
| `GetDupKeys(arg0, arg1)` | arg0: Integer (номер записи), arg1: Query | Empty | Заполняет Query списком продублированных первичных ключей в записи EME. Поля Query: `KEY` (`INT64`, значение ключа) и `COUNT` (Integer, число повторений). Ключи со значением 0 игнорируются. Читает все ключи записи единым блоком через `get_elementary_data_block` и сортирует перед подсчётом. Значение `COUNT` равно числу **дополнительных** вхождений ключа сверх первого: для ключа, встретившегося N раз, `COUNT = N - 1` (инициализация `1` + инкременты); ключ с единственным вхождением в результат не попадает. |
| `ResolveDupKeys(arg0)` | arg0: Integer (номер записи) | Integer | Устраняет дубликаты первичных ключей в записи EME: для каждого дубликата (обход с конца диапазона) генерируется новый ключ через `put_new_line_key`. Возвращает количество переназначенных ключей. Подробности каждого изменения (номер, строка, ключ) записываются в catastrophic-лог. **Внимание:** в текущей реализации цикла обхода (тело метода в `i_Replication.cpp`) счётчик стартует с `keys.size()` — на единицу больше последнего допустимого индекса вектора, поэтому первая итерация читает элемент за пределами вектора ключей. Поведение этого чтения не определено; на практике оно обычно безвредно (попадает либо на нулевой ключ, игнорируемый фильтром `*it == 0`, либо не находит совпадения в `mapDups`), но полагаться на это нельзя. Рекомендация автору C++: начать цикл с `keys.size() - 1`. |
