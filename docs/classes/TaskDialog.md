...
Объект создаётся с обязательным указанием текста сообщения, инструкции и заголовка. В расширенных формах конструктора можно сразу задать стандартные кнопки, опции отображения, текст подвала и список команд. После создания объект настраивается методами (текст, иконки, кнопки, команды, радиокнопки, прогресс) и отображается модальным вызовом `DoModal`, после которого доступны идентификаторы выбранной команды, радиокнопки и состояние флажка подтверждения.

Класс `TaskDialog` в языке EME-L используется в прикладных скриптах для организации информативных диалогов с пользователем, запроса подтверждений, выбора варианта действия из нескольких команд, отображения хода длительных операций и других сценариев, где возможностей `MessageBox` недостаточно.

Класс `TaskDialog` доступен только при включённой поддержке Task Dialog в ядре (C++ символ препроцессора `EME_TASK_DIALOG`).
