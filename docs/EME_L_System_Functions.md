...
Выполняет и возвращает результат выражения EME-L arg1, если условие arg0 истинно, иначе выполняет и возвращает результат выражения arg2. Выражения передаются в виде текстовых строк и компилируются при выполнении.
...
**Возвращает**: `Variant` — результат выполнения выбранного выражения EME-L.
...
Выполняет жёсткое форматирование кода EME-L указанного класса с заданным размером отступа и режимом форматирования.
...
### is_exec — выполнение выражения EME-L

Выполняет выражение EME-L с возможностью передачи параметров, доступных через is_arg(). При ошибке компиляции или выполнения выражения генерирует исключение.
...
**Возвращает**: `Variant` — результат выполнения выражения EME-L.
...
' Проверить, выполняется ли код на транзакционном порту HTTP-сервера (TRUE — транзакционный порт) '
...
```EME-L
' Проверить семейство ОС (TRUE — выполнение на Linux) '
...
```EME-L
' Проверить наличие ключа командной строки /d_emel (TRUE — ключ установлен) '
...
```EME-L
' Проверить, является ли оператор администратором (TRUE — администратор) '
...
```EME-L
' Проверить пароль оператора (TRUE — пароль верный) '
result = is_password()
```
...
```EME-L
' Проверить, является ли текущий филиал центральным (TRUE — центральный филиал) '
result = is_center()
```
...
```EME-L
' Проверить, является ли текущий филиал главным (корневым) центром (TRUE — главный центр) '
result = is_root_center()
```
...
```EME-L
' Проверить, является ли текущий филиал локальным (промежуточным) центром (TRUE — локальный центр) '
result = is_node_center()
```
...
```EME-L
' Проверить, является ли текущий филиал обычным филиалом (TRUE — обычный филиал) '
result = is_filial()
```
...
```EME-L
' Проверить, является ли текущий филиал системным (TRUE — системный филиал) '
result = is_system_filial()
```
...
добавляет строку в историю операций. При двух параметрах используется запись OperationHistory с автоматическим определением текущего оператора. При четырёх параметрах вызывает устаревшую функцию add_history_line. args[2] и args[3] передаются только вместе (оба или ни одного).
...
```EME-L
' Проверить версию ядра (TRUE — версия ядра >= 30.5) '
...
```EME-L
' Проверить флаг архивации строки 5 записи 100 (TRUE — помечена) '
arc = is_eme_is_arc(100, 5)

' Проверить флаг архивации по объекту CEMERec (TRUE — помечена) '
arc = is_eme_is_arc(rec)
```
...
' Удалить с указанием пути архивной выгрузки '
...
```EME-L
' Проверить флаг READ_ONLY органа EditField1 (TRUE — только чтение) '
result = is_read_only("EditField1")

' Проверить флаг READ_ONLY текущего органа (TRUE — только чтение) '
result = is_read_only()
```
...
```EME-L
' Проверить выбор радиокнопки в группе (TRUE — выбрана вторая) '
result = is_selected("RadioGroup", 2)
```
...
```EME-L
' Проверить фокус ввода на органе управления (TRUE — фокус на EditField1) '
result = is_focus("EditField1")
```
...
```EME-L
' Проверить, было ли изменено значение колонки пользователем (TRUE — изменено) '
result = is_modified("QuantityColumn")
```
...
' Получить ссылку на родительский узел дерева (TRUE — есть родитель для рекурсивной вставки) '
hasParent = (parentRef <> NULL_REF)
```
...
' Получить JSON заказа с пересчётом в литры '
...
```EME-L
' Включить режим работы со срезами данных (TRUE — срезы включены) '
